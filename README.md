# TI.GPIO - A Linux based Python library for TI GPIO RPi header enabled platforms
...
TI GPIO Library package. The library has the same API as the RPi.GPIO
library for Raspberry Pi in order to provide an easy way to move applications
running on the Raspberry Pi to the TI board.

This document walks through what is contained in The TI GPIO library
package, how to configure the system and run the provided sample applications,
and the library API.

# Package Components

In addition to this document, the TI GPIO library package contains the
following:
...
The following describes the operation of each application under the `samples/`.:
...
To run these sample applications if TI.GPIO is added to the PYTHONPATH:
```shell
...
The TI GPIO library provides all public APIs provided by the RPi.GPIO
library. The following discusses the use of each API:

#### 1. Importing the libary

To import the TI.GPIO module use:
```python
...
TI.GPIO for existing code using the RPi library.
...
The mode must be one of GPIO.BOARD, GPIO.BCM, GPIO.SOC, or None.
...
#### 8. TI Board Information and library version

To get information about the TI module, use/read:

```python
GPIO.BOARD_INFO
```

This provides a Python dictionary with the following keys: RAM,
...
# Integration with gpiozero

Please follow [instructions for integrating the TI.GPIO library](patches/README.md)
with another popular python library `gpiozero`.
